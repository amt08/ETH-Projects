...
# only look where at least some obs
# first we look at rows where ALL vitals are missing:

# Visualise a bit -----
# 5143 to start
# 863 with basically no data
# 859 of these are first surgeries!
# 450 are definitive. A lot of first surgeries don't have any vitals!
# down to: 4569! More now! 

# now we have: 
n <- nrow(df_full)

#hist(sapply(df_full, function(x) {sum(is.na(x))/n}), xlab = "Missing percent", 
#     main = "Number of variables with their missing percent")


# Let's now visualise this: We still have kept all variables
missing_percent <- sapply(df_full, function(x) {sum(is.na(x))/n})
# Let's concentrate on just all with first:

all_var_names <- names(df_full)[grep("first", names(df_full))]

#vis_dat(df_full[ , all_var_names])
...
# IMPUTATION ---------
# when missing values AND previous surgery data is available

# Imputing # Forward-filling only after the 1st observation has been recorded
impute_help <- function(x) {
  for (j in names(x)) {
    if (any(is.na(x[, j]))) {x <- x %>% fill(j)}
  }
  x
}
                      
# Actual imputation
df_full_factorised_imputed <- df_full_factorised %>% group_by(research_case_id) %>% 
                          impute_help
# fills in up to 15%. But not thaaat much help
#hist(sapply(df_full_factorised, function(x) {sum(is.na(x))/n}) - 
#       sapply(df_full_factorised_imputed, function(x) {sum(is.na(x))/n}), 
#     main = "", xlab = '% decrease in missingness')
 
all_var_names <- names(df_full_factorised_imputed)[grep("first", names(df_full_factorised_imputed))]


# now we only look at those labelled! NB to impute with all available values!
df_full_factorised_imputed <- df_full_factorised_imputed %>% filter(flag == 1| flag == 0)
#vis_miss(df_full_factorised_imputed[, all_var_names])

# Other options: fill in definitive surgeries 
# Fill in with normal values?
# can ask what normal values are

# now? -------
missing_percent <- sapply(df_full_factorised_imputed, function(x) {sum(is.na(x))/n})
# what else? 
sapply(df_full_factorised_imputed, function(x) {sum(is.na(x))/n})
# what can we remove?
# when n = 1?
n_var <- names(df_full_factorised_imputed)[grep('_n', names(df_full_factorised_imputed))]
# n = NA, make it 0. 

df_full_factorised_imputed[, n_var][is.na(df_full_factorised_imputed[, n_var])] <- 0

is.nan.data.frame <- function(x) {
  do.call(cbind, lapply(x, is.nan))
}

df_full_factorised_imputed[is.nan(df_full_factorised_imputed)] <- NA
sapply(df_full_factorised_imputed, function(x) {sum(is.na(x))/n})


# T and temperatur: the same. Merge.
to_remove <- names(df_full_factorised_imputed)[grep("_Categ", names(df_full_factorised_imputed))]
t <- names(df_full_factorised_imputed)[grep("T_", names(df_full_factorised_imputed))]

df_full_factorised_imputed <- df_full_factorised_imputed %>% 
...
sum(complete.cases(as.matrix(df_full_factorised_imputed[, -c(1:13, ncol(df_full_factorised_imputed))]))) # 349

df <- df_full_factorised_imputed
...
if (impute == "hard") {
  ...
  save(df_all_imputed, file = 'df_all_imputed.RData')
}
...
var_to_remove <- c('Death', 'has_operation', 'multiple', 'is_last',
                   'hemo_is_involved', 'DtTmStop', 'DtTmStart')
...
df$SurgeryNr[is.na(df$SurgeryNr)] <- 0
sapply(df, function(x) {sum(is.na(x))/length(x)})
features_drop <- c('research_case_id', 'Startdat', 'Stopdat', 'surgery_type',
                   names(df)[grep('kurtosis', names(df))])
df <- df %>% dplyr::select(-features_drop)
#save(df, file = 'unimputed_all_cases.Rdata')


sum(complete.cases(df[, c(7:ncol(df))])) # 349!!!!

df_complete <- df[complete.cases(df[, 7:ncol(df)]) ,]
# right now: 337 to 254 variables!
# Can we maybe try and aggregate the n's? Mean n? Maybe 1 for vitals, 1 for bga and 1 for blut

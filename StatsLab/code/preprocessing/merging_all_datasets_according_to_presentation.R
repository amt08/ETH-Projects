...
# df_full_factorised <- df_full_factorised %>% filter(flag == 1| flag == 0)
...
# now? -------
missing_percent <- sapply(df_full_factorised, function(x) {sum(is.na(x))/n})
# what else? 
sapply(df_full_factorised, function(x) {sum(is.na(x))/n})
# what can we remove?
# when n = 1?
n_var <- names(df_full_factorised)[grep('_n', names(df_full_factorised))]
# n = NA, make it 0. 

df_full_factorised[, n_var][is.na(df_full_factorised[, n_var])] <- 0

#df_full_factorised <- df_full_factorised[, -12:13]
# 
...
#df_full_factorised[is.nan.data.frame(df_full_factorised)] <- NA
#sapply(df_full_factorised, function(x) {sum(is.na(x))/n})
...
sum(complete.cases(df[, c(7:ncol(df))])) # 201!!!!

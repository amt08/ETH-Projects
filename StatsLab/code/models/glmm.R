...
#load("unimputed_complete_cases.RData")
# don't impute if die without surgery
# don't impute if person in ICU and have 0 measurements
#sum(complete.cases(df[, 6:ncol(df)])) # 3406 from 1633 unique patients
# we take complete cases (as some are still missing)
...
# glm_mod <- glmer(flag~SurgeryNr+Temperatur_mean+Puls_mean+Diastolic_min+Systolic_max+(1|research_case_id),
#                  data = data.syn.caseid.scaled, family = 'binomial')
# 
# glm_mod2 <- glmer(flag~SurgeryNr+Temperatur_mean+
#                     Puls_mean+Diastolic_mean+Systolic_mean+
#                     n_vit + head_injury + max_ais + icu +
#                     (1|research_case_id), # the random effect...
#                   data = data.syn.caseid.scaled, family = 'binomial',
#                   control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
# 
# glm_mod3 <- glmer(flag~Temperatur_mean+
#                     Puls_mean+Diastolic_mean+Systolic_mean+
#                     n_vit + head_injury + max_ais + icu +
#                     (1|research_case_id), # the random effect...
#                   data = data.syn.caseid.scaled, family = 'binomial',
...
# y.pred <- predict(glm_mod, data.syn.caseid.scaled[, -2])
# 
# test <- roc(data.syn.caseid.scaled$flag, y.pred, levels = c("No", "Yes"), percent = TRUE)
...
cross_val <- function(x, k = 5, formula) {
  pred <- numeric(nrow(x))
...
# frm = as.formula('flag~SurgeryNr+Temperatur_mean+
#                     Puls_mean+Diastolic_mean+Systolic_mean+
#                     n_vit + head_injury + max_ais + icu +
#                     (1|research_case_id)')
# 
# pred <- cross_val(data.syn.caseid.scaled, formula = frm)
# 
# test <- roc(data.syn.caseid.scaled$flag, pred, levels = c("No", "Yes"), percent = TRUE)
# plot.roc(test, main="ROC", col = "red", print.auc = TRUE,  legacy.axes = TRUE, add = FALSE, asp = NA)

# some different models need to be used
# variable selection

# capture the AIC - how we can capture it.
#summary(glm_mod2)$AICtab[[1]]

# now for the many different models?
#names(data.syn.caseid.scaled)[-c(1:2)]

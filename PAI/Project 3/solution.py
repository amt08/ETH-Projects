...
class BO_algo(object):
    def __init__(self):
        ...
        self.sigma_f = 0.01
        self.sigma_v = 0.005
        self.constraint_model = GaussianProcessRegressor(kernel = WhiteKernel(noise_level=self.sigma_v ** 2) + ConstantKernel(3.5) *\
                                                         (RBF(length_scale=2)), random_state=1234)  # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor(kernel =  WhiteKernel(noise_level=self.sigma_f ** 2) + ConstantKernel(1.5) *\
                                                        (RBF(length_scale=1.5)), random_state=1234)  # TODO : GP model for your objective function
    ...
    def next_recommendation(self) -> np.ndarray:
        ...
        if len(self.previous_points) == 0: # we don't have points added yet return a random point
            # need to return correct shape
            # sample point from interval 0 - 6 => taken from optimize_acquisition_function
            x0 = domain_x[0, 0] + (domain_x[0, 1] - domain_x[0, 0]) * np.random.rand(1)
            x1 = domain_x[1, 0] + (domain_x[1, 1] - domain_x[1, 0]) * np.random.rand(1)
            return np.array([float(x0), float(x1)]).reshape((1, domain_x.shape[0]))
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        # compute predictive mean and variance at x as in paper
        mu_new, sigma_new = self.objective_model.predict(x.reshape(1, -1), return_std=True)
        sigma_new = float(sigma_new)
        
        # case when predictive variance is 0
        if(sigma_new == 0):
            return 0.0

        X = [point[:2] for point in self.previous_points]
          
        # compute f(x_best), where x_best = argmin_x f(x) as suggested in paper
        # current function values
        fct_values = np.array([point[2] for point in self.previous_points])
        t_min = fct_values[np.argmin(fct_values)]
        
        # z = t - mu / sigma
        num_improve = (t_min - mu_new)
        z =  num_improve / sigma_new

        # expected improvement
        ei = num_improve * norm.cdf(z) + sigma_new * norm.pdf(z)
        
        # compute predictive mean and variance for constraint model
        mu_c, sigma_c = self.constraint_model.predict(x.reshape(1, -1), return_std=True)

        # constraint: c(x) <= lambda => P(c <= lambda) = CDF(lambda)
        pr_c = norm.cdf(LAMBDA, loc=mu_c, scale=sigma_c)
        
        # constraint-weighted ei
        return float(ei * pr_c)
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        # TODO: enter your code here
        # fit the GPs with the data
        X_o = [point[:2] for point in self.previous_points]
        y_o = [point[2] for point in self.previous_points]
        y_c = [point[3] for point in self.previous_points]
        self.objective_model.fit(X_o, y_o)
        self.constraint_model.fit(X_o, y_c)
    ...
    def get_solution(self) -> np.ndarray:
        ...
        # TODO: enter your code here
        # get the x which minimizes f
        f_values = np.array([point[2] for point in self.previous_points])
        c_values = [point[3] for point in self.previous_points]

        # retrieve cases where c(x) > lambda => constraint is violated
        mask = [item > LAMBDA for item in c_values]
        # since you can have constraints > lambda, but whose function value is smaller than other relevant ones
        # constraints that are > lambda, should be even larger
        # in the function value to avoid being found by argmin
        f_values[mask] = np.inf 
        y_min_idx = np.argmin(f_values)
        x_opt =  [point[:2] for point in self.previous_points][y_min_idx]

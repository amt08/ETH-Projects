...
class Model(object):
    ...
    def predict(self, x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        gp_mean, gp_std = self.gpr_model.predict(x, return_std=True)

        # TODO: Use the GP posterior to form your predictions here
        
        gp_mean += self.offset
        mask = ((THRESHOLD - gp_mean >= 0) & (THRESHOLD - gp_mean <= 5))
        gp_mean[mask] = THRESHOLD
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        ...
        index = np.random.choice(train_data.shape[0], size = 1500, replace=False)

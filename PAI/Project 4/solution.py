...
class MLPCategoricalActor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        super().__init__()
        # outputs logits from the mlp created above (instantiation)
        self.logits_net = mlp([obs_dim] + list(hidden_sizes) + [act_dim], activation)

    def _distribution(self, obs):
        """Takes the observation and outputs a distribution over actions."""
        logits = self.logits_net(obs)
        return Categorical(logits=logits) # distribution over actions
    ...
    def _log_prob_from_distribution(self, pi, act):
        ...
        """
        return pi.log_prob(act) # log lik of the categorical distribution
    ...
    def forward(self, obs, act=None):
        ...
        # run MLP=> produce distribution over actions
...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            act_dist, _ = self.pi.forward(state)
            act = act_dist.sample()

            # compute the value function estimate at the given state
            val_fct_est = self.v.forward(state)

            # compute the log-likelihood of the ACTION, under the policy output distribution
            log_lik = self.pi._log_prob_from_distribution(act_dist, act)

        return act.item(), val_fct_est.item(), log_lik
...
class VPGBuffer:
    ...
    def store(self, obs, act, rew, val, logp):
        ...
        # append a single timestep result to the buffer
    ...
    def end_traj(self, last_val=0):
        ...
        rews = np.append(self.rew_buf[path_slice], last_val) # 0 if trajectory ended with no interruption
        vals = np.append(self.val_buf[path_slice], last_val)

        # TODO6: Implement computation of phi. implement the estimation of the advantage at each timestep (phi)
        ...
        # the policy gradients update should use the rewards to go instead of the full episode returns
        # we need to ignore noisy rewards which were not a consequence of the action under consideration

        # self.ptr = current index in buffer
    ...
    def get(self):
        ...
        # TODO7: Here it may help to normalize the values in self.phi_buf. Normalize the advantage values (to mean 0 and std=1)
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        # use the optimizers and the architectures given to update the policy neural networks at the end of each epoch

        #TODO8: Change the update rule to make use of the baseline instead of rewards-to-go. Change the
        # update rule for the policy to make use of the advantage function as a baseline.
        ...
        # log_lik_act = self.ac.pi._log_prob_from_distribution(self.ac.pi._distribution(obs), act)
        # loss = - torch.mean(log_lik_act * ret)

        log_lik_act = self.ac.pi._log_prob_from_distribution(self.ac.pi._distribution(obs), act)
        loss = -(log_lik_act * phi).sum()
        loss.backward()
    ...
    def get_action(self, obs):
        ...
        return self.ac.pi._distribution(obs).sample().item()

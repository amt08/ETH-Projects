...
class Model(object):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    current_logits, batch_log_prior, batch_log_posterior = self.network(batch_x)

                    # NLL
                    loss = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
                    loss += (batch_log_posterior - batch_log_prior) / num_batches
    ...
    def predict(self, data_loader: torch.utils.data.DataLoader) -> np.ndarray:
        ...
        for batch_x, batch_y in data_loader:
            ...
            # for each batch, append the predicted probabilities
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        sigma1 = 1.5
        sigma2 = 0.5
        sigma_mix = pi ** 2 * torch.tensor(sigma1) + (1-pi) ** 2 * torch.tensor(sigma2)
        ...
        self.prior = UnivariateGaussian(mu=torch.tensor(0.0), sigma=torch.tensor(sigma_mix), n_sam = torch.Size([1]))
        ...
        # initialize mu and rho for the weights of this layer
        self.w_mu = nn.Parameter(torch.zeros(self.out_features, self.in_features))
        initial = math.sqrt(6 / self.in_features)
        # fill in weights
        self.w_mu.data.normal_(0.0, initial)
        
        self.w_rho = nn.Parameter(torch.zeros(self.out_features, self.in_features).normal_(-6, 0.5))
        self.weights_var_posterior = MultivariateDiagonalGaussian(mu=self.w_mu, rho=self.w_rho)
        ...
        if self.use_bias:
            ...
            # initialize mu and rho for the bias of this layer
            self.b_mu = nn.Parameter(torch.zeros(self.out_features))
            # fill in bias
            self.w_mu.data.normal_(0.0, initial)
            self.b_rho = nn.Parameter(torch.zeros(self.out_features).normal_(-6, 0.5))
            
            self.bias_var_posterior = MultivariateDiagonalGaussian(mu=self.b_mu, rho=self.b_rho)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        w_epsilon = UnivariateGaussian(mu=torch.tensor(0.0), sigma=torch.tensor(1.0), n_sam=self.w_mu.shape).sample()
        
        # monte carlo sample drawn from the variational posterior (w_mu, w_rho)
        weights = self.w_mu + F.softplus(self.w_rho) * w_epsilon
        
        # as in paper Sec 3.2
        log_prior = self.prior.log_likelihood(weights)
        
        w_var_post = Normal(self.w_mu, F.softplus(self.w_rho))
        log_variational_posterior = w_var_post.log_prob(weights).sum()
        
        if self.use_bias:

            b_epsilon = UnivariateGaussian(mu=torch.tensor(0.0), sigma=torch.tensor(1.0), n_sam=self.b_mu.shape).sample()
            
            bias = self.b_mu + F.softplus(self.b_rho) * b_epsilon
            log_prior += self.prior.log_likelihood(bias)
            
            # q distribution for the bias
            b_var_post = Normal(self.b_mu, F.softplus(self.b_rho))
            
            # add the log-var post probab. for the bias to the weights
            log_variational_posterior += b_var_post.log_prob(bias).sum()
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            new_features, prior, var_post = current_layer(current_features)
            # add log_prior and variational posterior output of each layer
            log_prior += prior
            log_variational_posterior += var_post
            # we don't add ReLU to the last layer
    ...
    # per batch (includes softmax for last layer)
    def predict_probabilities(self, x: torch.Tensor, num_mc_samples: int = 10) -> torch.Tensor:
        ...
        :param x: Features to predict on, float tensor of shape (batch_size, in_features) // 128x784
        ...
        return estimated_probability # 128x10
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, sigma: torch.Tensor, n_sam = torch.Size([10, 784])):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        # log lik of a univariate gaussian
        log_lik = -(values.size()[0]/2) * torch.log(2 * np.pi * (self.sigma ** 2)) -\
                    (1/(2 * (self.sigma ** 2))) * torch.sum((values - self.mu) ** 2)

        return log_lik

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        # sample from the univariate gaussian
        univg = Normal(self.mu, self.sigma)
        return univg.sample(sample_shape=self.n_sam)
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        mvrnorm = Normal(self.mu, self.sigma)
        return mvrnorm.log_prob(values).sum()

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        mvrnorm = Normal(self.mu, self.sigma)
        return mvrnorm.sample()
